...
const ALL_FRAMES_FOLDER = path.join(EXTRACTED_FRAMES_FOLDER, "all_frames");
const SORTED_FRAMES_FOLDER = path.join(
...
async function main() {
	...
	if (!fs.existsSync(ALL_FRAMES_FOLDER)) {
		fs.mkdirSync(ALL_FRAMES_FOLDER, { recursive: true });
	}
	if (!fs.existsSync(SORTED_FRAMES_FOLDER)) {
		fs.mkdirSync(SORTED_FRAMES_FOLDER, { recursive: true });
	}
	...
	try {
		...
		console.log(`📁 All frames are available in: ${ALL_FRAMES_FOLDER}`);
		console.log(
			`📁 Frames sorted by video are available in: ${SORTED_FRAMES_FOLDER}`
		);
	} catch (error) {
...
function extractFramesFromSingleVideo(
	videoPath,
	startFrameNumber,
	videoFileName
) {
	return new Promise((resolve, reject) => {
		...
		// Extract meaningful name from video filename
		// Format: output_1_imagename.mp4 -> imagename
		const baseVideoName = path.parse(videoFileName).name;
		const videoNameParts = baseVideoName.split("_");
		const meaningfulName =
			videoNameParts.length >= 3
				? videoNameParts.slice(2).join("_")
				: baseVideoName;

		// Create subfolder for this video in sorted frames folder
		const videoSubfolder = path.join(SORTED_FRAMES_FOLDER, meaningfulName);
		if (!fs.existsSync(videoSubfolder)) {
			fs.mkdirSync(videoSubfolder, { recursive: true });
		}
		...
		ffmpeg.ffprobe(videoPath, (err, metadata) => {
			...
			ffmpeg(videoPath)
				...
				.on("end", () => {
					...
					// Pattern for sorted folder (individual numbering per video)
					const sortedFramesPattern = path.join(
						videoSubfolder,
						`${meaningfulName}_frame_%06d.jpg`
					);

					let frameCount = 0;

					// First, extract frames to the sorted folder
					ffmpeg(videoPath)
						.outputOptions([
							`-vf ${filterOptions}`,
							"-vsync 0",
						...
						.on("end", () => {
							// Count extracted frames in sorted folder
							const sortedFrames = fs
								.readdirSync(videoSubfolder)
								.filter((file) => file.endsWith(".jpg"))
								.sort();

							frameCount = sortedFrames.length;
							console.log(
								`\n✅ Frames extracted to sorted folder: ${frameCount}`
							);

							// Now copy frames to all_frames folder with continuous numbering
							console.log(
								`📋 Copying frames to all_frames folder...`
							);

							sortedFrames.forEach((frameFile, index) => {
								const sourcePath = path.join(
									videoSubfolder,
									frameFile
								);
								const globalFrameNumber =
									startFrameNumber + index + 1;
								const destFileName = `${meaningfulName}_frame_${String(
									globalFrameNumber
								).padStart(6, "0")}.jpg`;
							...
							console.log(
								`✅ ${frameCount} frames copied to all_frames folder`
							);
							resolve(frameCount);
						})
						.on("error", (err) => reject(err))
